...
$mysqli = new mysqli("localhost", "root", "root", "rendezview");
	...
	<div id="render" class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
		...
		<div class="modal-body">
			...
			<div class="panel panel-default recipients">
				...
				<div class="panel-body">
					@foreach($getAttendees as $value)
					<div class="recipient" title="{{ $value["username"] }}
					{{ $value["email"] }}">
					{{ $value["username"] }}
					<div class="email">
						{{ $value["email"] }}
...
// $getDelegateCount = $getDelegate->rowCount();
		...
		<p>
			...
			<div class="form-group">
				...
				<select class="form-control" name="delegate" id="delegate">
					@foreach($getDelegateID as $value)
					<option value="{{{ $value["username"] }}}|{{{ $value["attendees_user_id"] }}}">{{{ Lang::get($value["username"]) }}}</option>
					@endforeach
				</select>
			</div>
