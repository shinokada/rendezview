...
-----------------*/
    ...
    $timelinePanel = ('.timeline-panel'),
        $message,
        _deleteBtn = '.delete-appt',
        _editUpcomingBtn = '.edithref';
        _messageOpen = 'message-open',
    ...
    openModal = function (apptID) {
        ...
        $edit
            .empty()
            .load(_eventUrl, function (e) {
                $(this).modal('show');
            });
        scope.on('click', '.modal-backdrop', function (e) {
            alert(this);
        });
    };

    //listen for delete button click
    $modalScope.on('click', _deleteBtn, function (e) {
        $message = $modalScope.find('.message');
        console.log(this);
        if ($message.hasClass(_messageOpen)){
            console.log('trigger delete call');
            $(this).closest('form').submit();
        }
        e.preventDefault();
        $message.slideDown().addClass(_messageOpen);
    });
...
-----------------*/
    ...
    scope.on('click', '.timeline-panel', function (e){
        ...
        var $this = $(this),
            anchor = $this.find('.roomhref'),
            roomhref = anchor.attr('href'),
            calDate = parseDate(anchor.attr('data-time'));
            $.cookie('dashToCal', calDate, { expires: 2, path: '/' });
            window.location.href = roomhref;
            // $this.find('')
        // var $this = $(this),
        //     roomHref = $this.find('.roomHref').attr('href');
        //     console.log(roomHref);
        //     window.open(roomHref);
    });
    scope.on('click', '.timeline-panel .edit-btn', function (e) {
        e.preventDefault();
        e.stopPropagation();
...
//         var mobileView = 'agendaDay';
//         $('#calendar').fullCalendar('changeView', mobileView );
//     }
// }

// setTimeout(initCalendarView, 500);
// initCalendarView();

});

// $('data-table-large').dataTable({
//     "bProcessing": true,
//     "bServerSide": true,
//     "sAjaxSource": "/datatables" + window.location.pathname
// });
